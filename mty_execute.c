	...
	if (cmd.fd)
	{
		while (getline(&cmd.line, &bufsize, cmd.fd) != -1)
		{
			...
			if (token == NULL)
			{
				...
				continue;
			val = strtok(NULL, " \n\t\r");
			r = getopc(&stack, token, val, c_line);
			if (r == 1) /** getopc return 1 when the value is not digit */
				push_error(cmd.fd, cmd.line, stack, c_line); /** print push error*/
			else if (r == -1) /** getopc return -1 if not the instruction */
				hand_error(cmd.fd, cmd.line, stack, token, c_line);
...
/**
 * getopc - function that get the option function
